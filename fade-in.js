...
const observer3 = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
        if (entry.isIntersecting) {
            entry.target.classList.add('SeenElement-3');
            observer3.unobserve(entry.target);
        }
    });
}, { threshold: getThreshold() });

const hiddenElements3 = document.querySelectorAll('.HiddenElement-3');
hiddenElements3.forEach((hiddenElements3) => observer3.observe(hiddenElements3));
...
// Add event listener to update the thresholds when the screen is resized ... Dit krijg ik niet voor elkaar
